class Tag < ActiveRecord::Base
  ...
  def self.search_and_tag(tag_name)
    talks = Talk.ferret_find(tag_name, 1000)
    return false if talks.blank?
  ...
  def self.find_popular(n=50)
    ...
    sql << " where t.id = j.tag_id and t.name is not null group by j.tag_id order by popularity desc limit #{n}"
